        ...
        <div class="card">
            <div class="card-header header-elements-inline">
                ...
                <div class="header-elements">
                    @can('create-sector')
                        <a data-target="addModal" data-action="{{route('store-sector')}}" id="addBtn" href="#"
            ...
            <table class="table datatable-basic">
                ...
                @foreach($sectors as $sector)
                    <tr>
                        <td>{{$sector->name}}</td>
                        <td>{{ showStatus($sector->is_active)}}</td>
                        <td>
                            <div class="d-flex">
                                @can('edit-sector')
                                    <a id="editBtn" href="" data-target="editModal"
                                       data-action="{{route('update-sector')}}"
                                       data-id="{{$sector->id}}" data-url="{{route('get-sector')}}"
                                       class="text-primary mr-1 mr-1 edit-btn "><i
                                            class="far fa-edit"></i></a>
                                @endcan

                                @can('delete-bill')
                                    <a href="javascript:void(0)" data-url="{{route('changeStatus-sector')}}"
                                       data-status='0'
                                       data-label="inactive"
                                       data-id="{{$sector->id}}"
                                       class=" text-danger mr-1 change-status-record {{($sector->is_active == '1') ? '' : 'd-none'}}"
                                    ...
                                    <a href="javascript:void(0)" data-url="{{route('changeStatus-sector')}}"
                                       ...
                                       data-label="active"
                                       data-id="{{$sector->id}}"
                                       class="text-success mr-1 change-status-record {{($sector->is_active == '0') ? '' : 'd-none'}}"
                                       title="Active Record"><i class="fas fa-exchange-alt"></i></a>


                                    <a href="javascript:void(0)" data-url="{{route('delete-sector')}}"
                                       data-id="{{$sector->id}}"
