...
class StampDutyBill extends Model
{
    use HasFactory;

    protected $table = 'stamp_duty_bills';
    protected $fillable = [
        'bill_number',
        'allotee_id',
        'bank_id',
    ...
    {
        $currentYear = date('Y');
        $lastBill = Bill::orderBy('id', 'desc')->where('is_stamp_duty',true)
            ->first();
         if ($lastBill) {
    ...
    {
        return $this->hasMany(StampDutyBillCharge::class);
    }

    public function transaction()
    {
        return $this->hasOne(StampDutyBillTransaction::class);
    }
